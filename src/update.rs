...
fn run_createrepo() -> Result<(), String> {
    ...
    if has_local {
        ...
        run_cmd(Command::new("createrepo_c").arg("--update").arg(".").current_dir("rpm"))?;
    } else {
        ...
        if has_nix {
            ...
            run_cmd(Command::new("nix-shell")
                .args(["-p", "createrepo_c", "--run", "createrepo_c --update ."])
                .current_dir("rpm"))?;
        } else {
            println!("Warning: createrepo_c and nix-shell not found, skipping RPM repository indexing.");
        }
    }
    Ok(())
}
        ...
        if let Ok(output) = key_check {
            if output.status.success() {
                ...
                run_cmd(Command::new("gpg")
                    .args(["--batch", "--yes", "--default-key", signing_key, "--detach-sign", "--armor", "repodata/repomd.xml"])
                    .current_dir("rpm"))?;
